...
# =============================
# Sidebar (with explicit keys to avoid KeyError)
# =============================
with st.sidebar:
    ...
    milestones_url = st.text_input("Milestones URL", value=DEFAULT_MILESTONES_URL, key="milestones_url")
    dependencies_url = st.text_input("Dependencies URL", value=DEFAULT_DEPENDENCIES_URL, key="dependencies_url")
    auth_header = st.text_input("Authorization header (optional)", value=default_auth, type="password", key="auth_header")

    completion_statuses = st.multiselect("Completion statuses count as ACTUAL", options=DEFAULT_COMPLETION_STATUSES, default=DEFAULT_COMPLETION_STATUSES, key="completion_statuses")

    st.markdown("---")
    st.markdown("**Column Mapping**")
    ID_COL = st.text_input("Internal ID column", ID_COL, key="map_internal_id")
    TEMPLATE_COL = st.text_input("Template column", TEMPLATE_COL, key="map_template")
    STATUS_COL = st.text_input("Status column", STATUS_COL, key="map_status")
    MILESTONE_COL = st.text_input("Milestone column", MILESTONE_COL, key="map_milestone")
    DATE_COL = st.text_input("Date column", DATE_COL, key="map_date")
    FORECAST_FLAG_COL = st.text_input("Is Forecast column", FORECAST_FLAG_COL, key="map_is_forecast")
    DESIGNER_COL = st.text_input("Designer column", DESIGNER_COL, key="map_designer")
    DEPENDENCY_COL = st.text_input("Dependency column", DEPENDENCY_COL, key="map_dependency")
...
if ms_df.empty:
    ...
    with tabs[2]:
        ...
        pick = st.multiselect("Choose milestones", options=sorted(ms_df[MILESTONE_COL].dropna().unique()), default=[m for m in ["GA","DD","Legal Access","MS6"] if m in ms_df[MILESTONE_COL].unique()], key="pick_milestones")
        for m in pick:
    ...
    with tabs[4]:
        ...
        picked = st.multiselect("Milestones to forecast", options=sorted(ms_df[MILESTONE_COL].dropna().unique()), default=[m for m in ["GA","DD","Legal Access","MS6"] if m in ms_df[MILESTONE_COL].unique()], key="picked_forecast")
        for m in picked:
            with st.expander(f"{m}"):
                taa = weekly_targets_vs_actuals(ms_df, m, completion_statuses)
